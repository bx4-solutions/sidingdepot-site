  ...
  return (
    <section className="relative isolate overflow-hidden" style={{ minHeight: "92vh" }}>
      ...
      <div
        ...
        style={{ background: "linear-gradient(105deg, rgba(30,42,58,0.92) 0%, rgba(30,42,58,0.70) 50%, rgba(30,42,58,0.30) 100%)" }}
      />
        ...
        <div className="max-w-3xl">
          <div className="flex items-center justify-center gap-3 mb-6">
            <span
              ...
              style={{ background: "rgba(179,209,51,0.15)", border: "1px solid #B3D133", color: "#B3D133" }}
            >
          ...
          <h1 className="font-display text-white leading-[0.9] text-4xl sm:text-5xl lg:text-7xl xl:text-8xl">
            Windows That
            <br />
            <span style={{ color: "#B3D133" }}>Pay For</span>
            <br />
            Themselves.
          </h1>
...
function TrustStrip() {
  ...
  const items = [
    ...
    { label: "4.7★", sublabel: "261 GuildQuality Reviews", href: "https://www.guildquality.com/profile/siding-depot-llc" },
    { label: `${rating}★`, sublabel: `${totalReviews} Google Reviews`, href: "https://www.google.com/search?q=Siding+Depot+Marietta+GA+reviews" },
    { label: "4.9★", sublabel: "91 Thumbtack Reviews", href: "https://www.thumbtack.com/ga/marietta/vinyl-siding-install/siding-depot-llc/service/482073672544780291" },
    ...
    <div style={{ background: "#1e2a3a", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="flex flex-wrap items-center justify-center lg:justify-between gap-x-8 gap-y-0 py-0">
          {items.map((item, i) => {
            const inner = (
              <div key={item.label} className="flex items-center gap-3 py-4 px-2"
            ...
            return item.href ? (
              <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" className="hover:opacity-80 transition-opacity">{inner}</a>
...
function StatItem({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center text-center px-6 py-8">
      <span ref={ref} className="font-display text-3xl sm:text-5xl lg:text-6xl" style={{ color: "#B3D133" }}>
        {count.toLocaleString()}{suffix}
      </span>
      <span className="mt-1 text-sm font-semibold uppercase tracking-wider text-white/60">{label}</span>
    </div>
  );
}
    ...
    <section style={{ background: SD_NAVY }} className="border-t border-white/5">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-white/8">
          {STATS_DATA.map((s) => <StatItem key={s.label} {...s} />)}
        </div>
      </div>
    </section>
  );
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8 grid gap-16 lg:grid-cols-2 items-center">
        <div className="relative">
          <div className="absolute -inset-4 rounded-3xl opacity-10"
            style={{ background: "linear-gradient(135deg, #B3D133, transparent)" }} aria-hidden />
          <img src="/projects/project-1.webp"
          ...
          <div className="absolute -bottom-6 -right-4 rounded-2xl px-6 py-4 shadow-xl text-center"
            style={{ background: "#1e2a3a", border: "1px solid rgba(179,209,51,0.3)" }}>
            <p className="font-display text-4xl" style={{ color: "#B3D133" }}>Up to 31%</p>
            <p className="text-xs text-white/60 uppercase tracking-widest font-bold mt-0.5">Energy Savings</p>
          </div>
        </div>
        <div>
          ...
          <p className="mt-6 text-gray-600 leading-relaxed text-lg">
            A premium window installed incorrectly performs no better than a builder-grade one. Air infiltration,
            water intrusion, and frame gap failures all come from poor installation — not the window itself.
            <strong> The install is 60% of the result.</strong>
          </p>
          <p className="mt-4 text-gray-600 leading-relaxed">
            Every Siding Depot window is installed with precision-fitted flashing, foam-seal perimeters, and
            factory-matched trim — the complete system that delivers the energy savings the manufacturer
            actually promises. That's why our installs come with a written 15-year labor warranty on top of
            the manufacturer's material coverage.
          </p>
          <div className="mt-8 rounded-xl overflow-hidden border border-gray-200 overflow-x-auto">
            <div className="min-w-[340px]">
              <div className="grid grid-cols-3 text-xs font-bold uppercase tracking-wider bg-gray-50 px-3 py-3 text-gray-500">
                ...
                <span className="text-center" style={{ color: SD_LIME_TEXT }}>Siding Depot</span>
              </div>
              {[
                ...
                <div key={factor} className="grid grid-cols-3 px-3 py-3 border-t border-gray-100 text-xs sm:text-sm">
                  <span className="font-medium text-gray-700">{factor}</span>
                  <span className="text-center text-gray-400">{standard}</span>
                  <span className="text-center font-semibold" style={{ color: SD_LIME_TEXT }}>{elite}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="max-w-2xl mb-14">
          ...
          <p className="mt-5 text-white/55 leading-relaxed">
            Every window we install includes Low-E double-pane glass, argon-gas fill, and energy-rated
            frames — engineered for Georgia's heat and humidity. Click to explore each style.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {WINDOW_TYPES.map((t) => {
            ...
            return (
              <button key={t.id} type="button" onClick={() => setActive(isActive ? null : t.id)}
                className="group relative overflow-hidden rounded-2xl text-left"
                style={{ aspectRatio: "3/4", outline: isActive ? "2px solid #B3D133" : "none", outlineOffset: 2 }}>
                <img src={t.image} alt={t.title}
                  className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105"
                  loading="lazy" />
                <div className="absolute inset-0 transition-opacity duration-300 group-hover:opacity-0"
                  style={{ background: "linear-gradient(to top, rgba(30,42,58,0.88) 0%, rgba(30,42,58,0.20) 60%, transparent 100%)" }} />
                ...
                <div className="relative z-10 h-full flex flex-col justify-end p-5">
                  {isActive && (
                    <span className="absolute top-4 right-4 text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"
                      style={{ background: "#B3D133", color: "#1e2a3a" }}>Selected</span>
                  )}
                  <p className="text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color: "#B3D133" }}>{t.subtitle}</p>
                  ...
                  <span className="mt-3 inline-flex items-center gap-1 text-xs font-bold" style={{ color: "#B3D133" }}>
        ...
        {activeType && (
          <div className="mt-6 rounded-2xl p-6 grid sm:grid-cols-[200px_1fr] gap-6 items-center"
            style={{ background: "rgba(179,209,51,0.06)", border: "1px solid rgba(179,209,51,0.2)" }}>
            <img src={activeType.image} alt={activeType.title}
              className="rounded-xl w-full object-cover" style={{ aspectRatio: "4/3" }} />
            <div>
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "#B3D133" }}>{activeType.subtitle}</p>
...
/* ---------------------------------------------------------------- */
const PROCESS = [
  { num: "01", title: "Free In-Home Assessment", desc: "We visit your home, measure every opening, assess frame condition, and identify where air and heat are escaping. You receive a written findings report with energy-loss estimates — at no charge.", icon: Users },
  { num: "02", title: "Custom Window Selection", desc: "We match window style, glass package (Low-E, argon, triple-pane options), and frame color to your home's architecture and HOA requirements. Written proposal with exact specifications before you commit.", icon: FileText },
  { num: "03", title: "Factory-Direct Order", desc: "Every window is custom-measured and factory-ordered to your exact opening size. No cutting, trimming, or shimming to force a fit — each unit arrives ready for a precision install.", icon: Award },
  { num: "04", title: "Professional Installation", desc: "Our W-2 crews remove the old window, inspect and repair the rough opening, install full-perimeter flashing, foam-seal the frame, and fit the new window to manufacturer specs. Interior and exterior trim matched and finished same day.", icon: ShieldCheck },
  { num: "05", title: "Energy Test & Final Walkthrough", desc: "We check every window for air infiltration, seal integrity, and smooth operation. You receive operating instructions, warranty documentation, and Energy Star certification paperwork before we leave.", icon: CheckCircle2 },
] as const;
    ...
    <section className="py-24 lg:py-32 bg-white overflow-hidden">
      ...
      <div className="mx-auto max-w-6xl px-4 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-20">
          ...
          <h2 className="font-display text-4xl sm:text-5xl text-sd-navy leading-tight">5 Steps. Zero Surprises.</h2>
          <p className="mt-4 text-gray-500 leading-relaxed">
            Most window jobs fail at steps 3 and 4 — rushed installs, no flashing, no air seal. We don't cut those corners.
          </p>
        </div>
        <div className="hidden lg:block relative">
          <div className="absolute left-1/2 -translate-x-1/2 top-[88px] bottom-[88px] w-0.5"
            style={{ background: "linear-gradient(to bottom, #B3D133 0%, #D4EC5A 50%, #B3D133 100%)", opacity: 0.35 }} />
          {PROCESS.map((step, i) => {
            const Icon = step.icon;
            const isTop = i % 2 === 0;
            return (
              <WPStepZigzag key={step.num} step={step} Icon={Icon} isTop={isTop}
                delay={`${i * 0.11}s`} index={i} total={PROCESS.length} />
            );
          })}
        </div>
        <div className="lg:hidden flex flex-col gap-0">
          {PROCESS.map((step, i) => {
            const Icon = step.icon;
            return <WPStepMobile key={step.num} step={step} Icon={Icon} delay={`${i * 0.09}s`} index={i} total={PROCESS.length} />;
          })}
        </div>
      </div>
    </section>
...
function WPStepZigzag({ step, Icon, isTop, delay, index, total }: {
  step: (typeof PROCESS)[number]; Icon: React.ElementType; isTop: boolean; delay: string; index: number; total: number;
}) {
  ...
  return (
    <div ref={ref} className="relative" style={{ height: 220, marginTop: index === 0 ? 0 : -60 }}>
      <div data-visible={vis} className={`wp-step ${isTop ? "wp-top" : "wp-bot"} absolute w-[44%]`}
        style={{ [isTop ? "right" : "left"]: "54%", ...(isTop ? { top: 0 } : { bottom: 0 }), animationDelay: delay }}>
        <div className="rounded-2xl p-5 h-full"
          style={{ background: "#f9fafb", border: "1px solid #e5e7eb", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          <div className="flex items-center gap-3 mb-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-full font-display text-sm font-bold shrink-0"
              style={{ background: "#B3D133", color: "#1e2a3a" }}>{step.num}</span>
      ...
      <div data-visible={vis} className="wp-dot absolute left-1/2 -translate-x-1/2"
      ...
      <div className="absolute left-1/2 -translate-x-1/2"
        style={{ width: "calc(44% + 10px)", height: 2, background: "rgba(179,209,51,0.3)",
          top: isTop ? "auto" : 10, bottom: isTop ? 10 : "auto", [isTop ? "right" : "left"]: "50%", transformOrigin: "left center" }} />
    </div>
  );
}

function WPStepMobile({ step, Icon, delay, index, total }: {
  step: (typeof PROCESS)[number]; Icon: React.ElementType; delay: string; index: number; total: number;
}) {
  const ref = useRef<HTMLDivElement>(null);
  const [vis, setVis] = useState(false);
  useEffect(() => {
  ...
  return (
    <div ref={ref} className="relative flex gap-4 pb-6">
      <div className="flex flex-col items-center shrink-0" style={{ width: 36 }}>
        <div data-visible={vis} className="wp-dot h-9 w-9 rounded-full border-4 border-white flex items-center justify-center font-display text-xs font-bold shrink-0"
          style={{ background: "#B3D133", color: "#1e2a3a", boxShadow: "0 0 0 3px rgba(179,209,51,0.22)", animationDelay: delay }}>
          {step.num}
        </div>
        {!isLast && <div className="flex-1 w-0.5 mt-2" style={{ background: "rgba(179,209,51,0.3)", minHeight: 24 }} />}
      </div>
      <div data-visible={vis} className="wp-step wp-mob flex-1 rounded-2xl p-5"
        style={{ background: "#f9fafb", border: "1px solid #e5e7eb", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", animationDelay: delay }}>
...
/* ---------------------------------------------------------------- */
type ShowcasePair = { before: string; after: string; beforeAlt: string; afterAlt: string; title: string; description: string; };
const SHOWCASE_PAIRS: ReadonlyArray<ShowcasePair> = BEFORE_AFTER_PAIRS.map((p, i) => {
  ...
  return { ...p, title: project?.title ?? `Real Project #${i + 1}`,
    description: project?.description ?? "Full exterior transformation by Siding Depot — windows, trim and finishing." };
});
    ...
    <section style={{ background: "#1e2a3a" }} className="py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-14">
          <div>
            <span className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-4"
              style={{ background: "rgba(179,209,51,0.12)", color: "#B3D133" }}>Real Projects</span>
            <h2 className="font-display text-4xl sm:text-5xl text-white leading-tight">
              Before & After.
              <br />
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-white/40 font-mono">{index + 1}/{total}</span>
        ...
        <div className="grid lg:grid-cols-[1.6fr_1fr] gap-10 items-start">
          <div className="rounded-2xl overflow-hidden shadow-2xl">
            <BeforeAfterSlider key={index} before={current.before} after={current.after}
              beforeAlt={current.beforeAlt} afterAlt={current.afterAlt} />
          </div>
          <div className="flex flex-col justify-center py-4">
            <div className="flex items-center gap-2 mb-8">
              {SHOWCASE_PAIRS.map((_, i) => (
                <button key={i} type="button" onClick={() => setIndex(i)} aria-label={`Project ${i + 1}`}
                  className="rounded-full transition-all"
                  style={{ width: i === index ? 24 : 8, height: 8, background: i === index ? "#B3D133" : "rgba(255,255,255,0.2)" }} />
              ))}
            </div>
            ...
            <div className="mt-8 flex flex-col gap-3">
              <Link to="/projects" search={{ page: 1, tag: "All" }}
...
/* ---------------------------------------------------------------- */
const WHY_US = [
  { Icon: Zap, title: "Energy Star Certified Installer", desc: "We're authorized to install and certify Energy Star rated windows — unlocking federal and Georgia energy rebates your project may qualify for. Most installers can't offer this." },
  { Icon: ShieldCheck, title: "W-2 Installers — Zero Subcontracting", desc: "Every window is installed by a Siding Depot W-2 employee — trained, background-checked, insured. We never hand your project to a subcontractor the day it starts." },
  { Icon: Thermometer, title: "Built for Georgia's Heat & Humidity", desc: "Low-E double-pane glass with argon-gas fill blocks Georgia's radiant heat and keeps humidity out. Not a Northern-spec window installed in the South — the right product for your climate." },
  { Icon: FileText, title: "Written Estimates — Locked Price", desc: "Detailed, itemized written estimate before any work begins. The number we quote is the number you pay — no change orders, no mid-project surprises." },
  { Icon: Eye, title: "Precision Measurement & Custom Order", desc: "Every opening is measured twice. Every window is custom-ordered to your exact dimensions. No field-cutting, no shimming to cover a bad fit. The window fits because we measured right." },
  { Icon: Wind, title: "15-Year Workmanship Warranty", desc: "Our installation warranty covers air infiltration, seal failure, and operational defects for 15 years — on top of the manufacturer's glass and frame coverage. The most complete window warranty in North Atlanta." },
] as const;
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid lg:grid-cols-[1fr_1.4fr] gap-16 items-start">
          <div className="lg:sticky lg:top-28">
            <span className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-5"
              style={{ background: "#eef7c0", color: SD_LIME_TEXT }}>Why Siding Depot</span>
            <h2 className="font-display text-4xl sm:text-5xl text-sd-navy leading-tight">
              Six Reasons North Atlanta Homeowners Choose Us For Windows.
            </h2>
            <p className="mt-5 text-gray-500 leading-relaxed">
              We're not the cheapest window installer in the area. We're the one that actually delivers the energy savings the manufacturer promises.
            </p>
            <img src="/projects/project-3.webp" alt="Siding Depot window installation team"
              className="mt-8 rounded-2xl w-full object-cover shadow-lg" style={{ aspectRatio: "4/3" }} loading="lazy" />
          </div>
          <div className="flex flex-col gap-5">
            {WHY_US.map((item, i) => {
              const Icon = item.Icon;
              return (
                <div key={item.title} className="flex gap-5 p-5 rounded-2xl transition-colors hover:bg-gray-50"
                  style={{ border: "1px solid #e5e7eb" }}>
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "#eef7c0" }}>
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="relative rounded-3xl overflow-hidden" style={{ background: "#1e2a3a" }}>
          <div aria-hidden className="absolute inset-0 opacity-30"
            style={{ backgroundImage: "radial-gradient(circle at 20% 50%, rgba(179,209,51,0.15) 0%, transparent 60%), radial-gradient(circle at 80% 20%, rgba(179,209,51,0.08) 0%, transparent 50%)" }} />
          <div className="relative grid lg:grid-cols-2 gap-0 items-center">
            <div className="relative h-72 lg:h-full min-h-[400px] overflow-hidden">
              <img src="/projects/project-4.webp" alt="Beautiful home with new windows installed by Siding Depot"
                className="absolute inset-0 h-full w-full object-cover" style={{ objectPosition: "center top" }} loading="lazy" />
              <div className="absolute inset-0 lg:hidden"
                style={{ background: "linear-gradient(to top, #1e2a3a 0%, transparent 60%)" }} aria-hidden />
              <div className="absolute inset-0 hidden lg:block"
                style={{ background: "linear-gradient(to right, transparent 60%, #1e2a3a 100%)" }} aria-hidden />
            </div>
            <div className="relative px-8 py-12 lg:pl-4 lg:pr-12">
              <span className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-6"
                style={{ background: "rgba(179,209,51,0.12)", color: "#B3D133" }}>Free Assessment</span>
              ...
              <p className="text-white/65 leading-relaxed mb-4">
                Old windows with failed seals and no Low-E coating can add 20–31% to your summer cooling bill.
                Our free in-home assessment identifies exactly which windows are performing and which are quietly
                draining your energy — with no obligation to buy.
              </p>
              <p className="text-white/65 leading-relaxed mb-8">
                We respond within 24 hours, come to you on your schedule, and deliver a written report at no charge.
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-5">
                {["No-obligation assessment", "Written energy report", "Energy Star certified"].map((t) => (
...
function WindowsPage() {
  ...
  const mappedReviews = reviewsData?.reviews?.length
    ? reviewsData.reviews.filter((r) => r.text).map((r) => ({
        author_name: r.author_name, rating: r.rating, text: r.text as string,
