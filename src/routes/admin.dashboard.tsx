...
function AdminDashboard() {
  ...
  const blogStats = BLOG_POSTS.reduce(
    (acc, post) => {
  ...
  const filteredSitePages = allSitePages.filter((page) => {
    const matchesCity =
      !locationCityFilter || page.city.toLowerCase().includes(locationCityFilter.toLowerCase());
    const matchesService =
      !locationServiceFilter ||
      page.service.toLowerCase().includes(locationServiceFilter.toLowerCase());
    const matchesRoute =
      !routeSearch || page.route.toLowerCase().includes(routeSearch.toLowerCase());
  ...
  const filteredBlogPosts = BLOG_POSTS.filter((post) => {
    ...
    const matchesSearch =
      !blogSearch || post.title.toLowerCase().includes(blogSearch.toLowerCase());
  ...
  // Paginated data
  const paginatedPages = filteredSitePages.slice(
  ...
  const paginatedBlogs = filteredBlogPosts.slice(
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-sd-gray-text font-medium">
              Dashboard monitor of pages, locations, and blog publication states.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="bg-white border-none shadow-sm ring-1 ring-black/5">
            ...
            <CardContent>
              ...
              <p className="text-xs text-sd-gray-text mt-1">
                {STATIC_ROUTES.length} Static + {locationPages.length} Dynamic
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-none shadow-sm ring-1 ring-black/5">
            ...
            <CardContent>
              ...
              <div className="flex gap-2 mt-1">
                <Badge className="bg-sd-green text-sd-black text-[10px]">
                  {blogStats.published} Pub
                </Badge>
                <Badge className="bg-amber-100 text-amber-800 text-[10px]">
                  {blogStats.draft} Draft
                </Badge>
                <Badge className="bg-blue-100 text-blue-800 text-[10px]">
                  {blogStats.scheduled} Sched
                </Badge>
              </div>
            </CardContent>
          </Card>
            ...
            <CardHeader className="border-b border-gray-100">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg font-bold text-sd-navy">
                  ...
                  <TableBody>
                    {paginatedPages.map((page, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell>
                          <Badge
                            variant={page.type === "static" ? "secondary" : "outline"}
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-1">
                            <span className="capitalize text-[10px] text-sd-navy font-medium">
                              {page.city}
                            </span>
                            <span className="capitalize text-[9px] text-sd-gray-text">
                              {page.service}
                            </span>
                          </div>
                        </TableCell>
                    ...
                    {paginatedPages.length === 0 && (
                      <TableRow>
                        <TableCell
                          colSpan={3}
                          className="text-center py-8 text-sd-gray-text text-sm"
                        >
                        ...
                        <TableRow key={post.slug}>
                          <TableCell className="max-w-[200px]">
                            <Link
                              ...
                              className="font-bold text-sd-navy text-xs truncate hover:text-sd-green transition-colors"
                              title={post.title}
                            >
                              {post.title}
                            </Link>
