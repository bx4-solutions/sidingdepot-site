    ...
    <div className="min-h-screen bg-gray-50/50 p-8">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8 flex items-center gap-4">
          <Link to="/admin/dashboard" className="text-sd-gray-text hover:text-sd-black transition-colors">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 p-4 bg-white rounded-lg border border-sd-green/20 font-mono text-sm space-y-1.5">
                    <p><span className="text-sd-gray-text text-xs uppercase tracking-wider">URL</span><br />{window.location.origin}/admin/login</p>
                    <p><span className="text-sd-gray-text text-xs uppercase tracking-wider">Email</span><br />{success.email}</p>
                    <p><span className="text-sd-gray-text text-xs uppercase tracking-wider">Senha</span><br />{success.password}</p>
                  </div>
                  <Button onClick={copyCredentials} variant="outline" size="sm" className="mt-3 rounded-full">
                    {copied
          ...
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid gap-1.5">
                <Label htmlFor="u-name" className="text-xs font-bold uppercase tracking-wider text-sd-navy">
                  Nome Completo *
                </Label>
                <Input id="u-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="João Silva" required className="h-11" />
              </div>

              <div className="grid gap-1.5">
                <Label htmlFor="u-email" className="text-xs font-bold uppercase tracking-wider text-sd-navy">
                  Email *
                </Label>
                <Input id="u-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="joao@email.com" required className="h-11" />
              </div>

              <div className="grid gap-1.5">
                <Label htmlFor="u-password" className="text-xs font-bold uppercase tracking-wider text-sd-navy">
                  Senha *
                </Label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-sd-gray-text hover:text-sd-black">
                      {showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
                    </button>
                  </div>
                  <Button type="button" variant="outline" onClick={generatePassword} className="h-11 rounded-lg text-xs whitespace-nowrap">
                    Gerar senha
                  </Button>
                </div>
                <p className="text-[11px] text-sd-gray-text">Clique em "Gerar senha" para criar uma senha segura automaticamente.</p>
              </div>
              ...
              <Button type="submit" disabled={loading} className="w-full h-11 bg-sd-green text-sd-black font-bold hover:bg-sd-green-hover">
                {loading
