...
function SEODashboard() {
  ...
  const handleExport = (format: "csv" | "pdf") => {
    if (format === "csv") {
      ...
      const pageRows =
        metrics?.topPages?.map(
          (p: any) =>
            `Página,"${p.path}",${p.views},${p.avgTime},${p.bounceRate}%,${p.conversions},"${p.leadsBySource?.map((s: any) => `${s.source}: ${s.count}`).join(" | ") || ""}"`,
        ) || [];
      const blogRows =
        metrics?.blogStats?.topArticles?.map(
          (p: any) =>
            `Blog,"${p.title}",${p.views},${p.avgTime},${p.bounceRate}%,${p.conversion}%,"${p.keywords?.join(" | ") || ""}"`,
        ) || [];
      const clickRows =
        metrics?.clickEvents?.map(
          (p: any) =>
            `Botão,"${p.button}",${p.clicks},,,${p.conversion}%,"${p.sources?.map((s: any) => `${s.source}: ${s.count}`).join(" | ") || ""}"`,
        ) || [];
      ...
      printWindow.document.write(
        `<html><head><title>Relatório SEO</title></head><body><h1>Relatório SEO</h1><p>Período: ${dateRange.startDate} até ${dateRange.endDate}</p><pre>${JSON.stringify({ paginas: metrics?.topPages, blog: metrics?.blogStats?.topArticles, botoes: metrics?.clickEvents }, null, 2)}</pre></body></html>`,
      );
      ...
      <aside className="w-64 bg-[#131921] border-r border-white/10 flex flex-col h-screen sticky top-0 shrink-0">
        <div className="p-6 flex items-center gap-3">
          ...
          <span className="font-display text-2xl tracking-tight text-white uppercase">
            Siding Depot <span className="text-sd-green">SD</span>
          </span>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          ...
          <div className="pt-6 pb-2">
            <span className="px-3 text-[10px] font-black uppercase tracking-widest text-slate-300">
              {t("administration")}
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-bold capitalize">
              {activeView === "dashboard"
                ? t("dashboard")
                : t(
                    menuItems.find((i) => i.id === activeView)?.labelKey ||
                      adminItems.find((i) => i.id === activeView)?.labelKey ||
                      activeView.replace("-", " "),
                  )}
            </h2>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10">
              ...
              <Input
                ...
                onChange={(e) =>
                  setDateRange((current: any) => ({ ...current, startDate: e.target.value }))
                }
              ...
              <Input
                ...
                onChange={(e) =>
                  setDateRange((current: any) => ({ ...current, endDate: e.target.value }))
                }
                className="h-7 w-[130px] bg-transparent border-white/10 text-[10px] text-white"
              />
            </div>
            ...
            <div className="flex items-center gap-3 border-l border-white/10 pl-6">
              <div className="text-right hidden sm:block">
                ...
                <span className="text-[10px] font-black text-sd-green uppercase">
                  {userProfile?.role}
                </span>
              </div>
            ...
            {!sessionExists ? (
              <Card className="bg-[#131921] border-sd-green/20 p-12 text-center animate-in fade-in zoom-in duration-500">
                ...
                <p className="text-slate-200 mb-8 max-w-sm mx-auto">
                  {lang === "pt"
                    ...
                    : "Unable to detect your session. Please reconnect to view metrics."}
                </p>
                <Button
              ...
              <Card className="bg-[#131921] border-red-500/20 p-12 text-center animate-in fade-in duration-500">
                ...
                <p className="text-slate-200 mt-2">
                  {lang === "pt"
                    ? "Ocorreu uma falha intermitente na conexão com o servidor."
                    : "An intermittent server connection failure occurred."}
                </p>
                <Button
                  onClick={() => refetch()}
                  variant="outline"
                  ...
                  <div className="space-y-8 animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                      <KPICard
                        ...
                        value={metrics?.overview.totalConversions}
                        icon={MousePointer2}
                        color="sd-green"
                        diff={lang === "pt" ? "+12% vs anterior" : "+12% vs previous"}
                      />
                      <KPICard
                        ...
                        icon={TrendingUp}
                        color="sd-green"
                        diff={lang === "pt" ? "+0.5% vs anterior" : "+0.5% vs previous"}
                      />
                      <KPICard
                        title={t("callClicks")}
                        value={
                          metrics?.clickEvents?.find((e: any) =>
                            e.button.toLowerCase().includes("call"),
                          )?.clicks || 0
                        }
                        icon={Phone}
                        color="sd-green"
                        diff={lang === "pt" ? "Leads por telefone" : "Phone leads"}
                      />
                      <KPICard
                        title={t("appointments")}
                        value={metrics?.overview.appointments}
                        icon={CalendarCheck2}
                        color="sd-green"
                        diff={lang === "pt" ? "3 novos hoje" : "3 new today"}
                      />
                      <KPICard
                        title={t("chatInteractions")}
                        value={metrics?.overview.chatInteractions}
                        icon={MessageSquare}
                        color="sd-green"
                        diff={lang === "pt" ? "Interações 24h" : "24h interactions"}
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                      <MiniKPICard
                        ...
                        value={
                          metrics?.overview?.totalVisitors
                      ...
                      <MiniKPICard
                        ...
                        value={
                          metrics?.overview?.uniqueVisitors
                      ...
                      />
                      <MiniKPICard
                        title={t("pagesPerVisit")}
                        value={
                          metrics?.overview?.pagesPerVisit
                            ? String(metrics.overview.pagesPerVisit)
                            : "—"
                        }
                        diff=""
                      />
                      <MiniKPICard
                        title={t("bounceRate")}
                        value={
                          metrics?.overview?.bounceRate ? `${metrics.overview.bounceRate}%` : "—"
                        }
                        diff=""
                      />
                      <MiniKPICard
                        ...
                        value={
                          metrics?.overview?.avgSessionDuration &&
                          metrics.overview.avgSessionDuration !== "0s"
                            ? metrics.overview.avgSessionDuration
                            : "—"
                        }
                        diff=""
                    ...
                    <Card className="bg-[#131921] border-white/10">
                      <CardHeader>
                        ...
                        <CardDescription className="text-xs">
                          Série diária dentro do período selecionado
                        </CardDescription>
                      </CardHeader>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <Card className="lg:col-span-2 bg-[#131921] border-white/10">
                        <CardHeader className="flex flex-row items-center justify-between">
                          <CardTitle className="text-lg font-bold text-white">
                            Ranqueamento e Performance de Páginas
                          </CardTitle>
                          <Badge className="bg-sd-green/10 text-sd-green border-sd-green/20">
                            Site Interno
                          </Badge>
                        </CardHeader>
                        <CardContent>
                                ...
                                {metrics?.topPages?.map((p: any, i: number) => (
                                  <tr key={i} className="group hover:bg-white/5 transition-colors">
                                    <td className="py-3 px-2">
                                      <div className="flex items-center gap-2">
                                        <span className="text-sd-green font-bold w-4">
                                          #{i + 1}
                                        </span>
                                        <button
                                          type="button"
                      ...
                      <Card className="bg-[#131921] border-white/10">
                        ...
                        <CardHeader className="flex flex-row items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            <Badge variant="outline" className="border-sd-green/30 text-sd-green">
                              Top CTA: Hero
                            </Badge>
                            <Badge variant="outline" className="border-sd-green/30 text-sd-green">
                              Top SVC: Siding
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <Card className="bg-[#131921] border-white/10">
                        <CardHeader>
                          <CardTitle className="text-lg font-bold text-white uppercase tracking-tight">
                            Ranking de Cliques em Botões
                          </CardTitle>
                          <CardDescription className="text-xs">
                            CTAs mais convertidos e origem do clique
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          {metrics?.clickEvents?.map((ev: any, i: number) => (
                            <div
                              key={i}
                              className="p-4 bg-white/5 border border-white/10 rounded-xl group hover:border-sd-green/30 transition-all"
                            >
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <Badge className="bg-sd-green/20 text-sd-green border-0 text-xs font-black">
                                  {ev.conversion}% CONV.
                                </Badge>
                              </div>
                              <div className="flex items-center justify-between border-t border-white/5 pt-3">
                                <div className="flex gap-2">
                                  {ev.sources?.map((s: any, si: number) => (
                                    <span
                                      key={si}
                                      className="text-[9px] text-slate-400 bg-white/5 px-2 py-0.5 rounded border border-white/5"
                                    >
                                      {s.source}
                                    </span>
                                  ))}
                                ...
                                <div className="text-right">
                                  ...
                                  <span className="text-[10px] text-slate-500 ml-1 uppercase font-bold tracking-tighter">
                                    Cliques
                                  </span>
                                </div>
                              </div>
                      ...
                      <Card className="bg-[#131921] border-white/10">
                        <CardHeader>
                          <CardTitle className="text-lg font-bold text-white uppercase tracking-tight">
                            Competição de Permanência
                          </CardTitle>
                          <CardDescription className="text-xs">
                            Tempo médio: Top Páginas vs Resto do Site
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="h-[280px] flex flex-col justify-between pt-6">
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart
                                data={[
                                  {
                                    name: "Top Páginas",
                                    value: metrics?.timeComparison?.topPagesAvg || 0,
                                  },
                                  {
                                    name: "Média Geral",
                                    value: metrics?.timeComparison?.restOfSiteAvg || 0,
                                  },
                                ]}
                              >
                                <XAxis
                          ...
                          <div className="flex justify-around items-end pt-6 border-t border-white/5">
                            <div className="text-center">
                              <p className="text-2xl font-black text-sd-green">
                                {metrics?.timeComparison?.topPagesAvg}s
                              </p>
                            ...
                            <div className="text-center">
                              <p className="text-2xl font-black text-white/20">
                                {metrics?.timeComparison?.restOfSiteAvg}s
                              </p>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                                Média Site
                              </p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      <Card className="lg:col-span-2 bg-[#131921] border-white/10">
                        <CardHeader className="flex flex-row items-center justify-between">
                          <div>
                            <CardTitle className="text-lg font-bold text-white">
                              Performance de Artigos
                            </CardTitle>
                            <CardDescription className="text-xs">
                              Ranqueamento dos melhores posts do blog
                            </CardDescription>
                          </div>
                          <Badge className="bg-sd-green/10 text-sd-green border-sd-green/20">
                            Blog
                          ...
                          <div className="space-y-4">
                            {metrics?.blogStats?.topArticles?.map((art: any, i: number) => (
                              <button
                                ...
                                className="w-full flex items-center justify-between p-3 bg-white/5 rounded-lg group hover:bg-white/10 transition-colors text-left"
                              >
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[10px] text-slate-400">
                                      {art.views.toLocaleString()} views · {art.conversion}% conv.
                                    </p>
                                  </div>
                                </div>
                      ...
                      <Card className="bg-[#131921] border-white/10">
                        <CardHeader>
                          ...
                          <CardDescription className="text-xs">
                            Comparativo com médias do setor
                          </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                          {metrics?.blogStats?.marketBenchmarks?.map((bench: any, i: number) => (
                            <div key={i} className="space-y-2">
                              <div className="flex justify-between text-xs">
                                ...
                                <span className="text-sd-green font-black">
                                  {bench.value}%{" "}
                              ...
                              <div className="relative h-2 w-full bg-white/5 rounded-full overflow-hidden">
                                <div
                                  className="absolute top-0 left-0 h-full bg-sd-green rounded-full z-10"
                                  style={{ width: `${bench.value}%` }}
                                />
                                <div
                                  className="absolute top-0 left-[70%] h-full w-0.5 bg-white/20 z-20"
                                  title="Benchmark"
                                />
                              </div>
                            </div>
                          ...
                          <div className="pt-2 border-t border-white/5">
                            <p className="text-[10px] text-slate-400 italic">
                              Benchmarks baseados em dados de mercado para o segmento de
                              Construção/IA em 2026.
                            </p>
                          </div>
                        </CardContent>
                      </Card>
                    ...
                    <Card className="bg-[#131921] border-white/10">
                      ...
                      <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white/5 border border-white/10 rounded-xl">
                          <div>
                            <p className="text-[10px] font-black uppercase text-slate-300 mb-1">
                              E-mail
                            </p>
                          ...
                          <div>
                            ...
                            <Badge className="bg-sd-green/20 text-sd-green border-sd-green/30 uppercase">
                              {userProfile?.role}
                            </Badge>
                          </div>
                        </div>
                    ...
                    <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-yellow-400">
                          Integrações externas requerem configuração manual
                        </p>
                        <p className="text-xs text-slate-300 mt-1">
                          Google Search Console e GA4 não se conectam automaticamente. Siga as
                          instruções abaixo para ativá-los.
                        </p>
                      </div>
                    </div>
                      ...
                      <Card className="bg-[#131921] border-white/10">
                        <CardHeader className="flex flex-row items-center gap-4">
                          ...
                          <div>
                            ...
                            <CardDescription>
                              Status de indexação e performance orgânica
                            </CardDescription>
                          </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          ...
                          <div className="p-3 bg-sd-black/40 border border-white/5 rounded-lg text-[11px] text-slate-300 space-y-1">
                            ...
                            <p>
                              1. Acesse{" "}
                              <span className="text-sd-green font-mono">
                                search.google.com/search-console
                              </span>
                            </p>
                            <p>
                              2. Adicione a propriedade{" "}
                              <span className="text-sd-green font-mono">sidingdepot.com</span>
                            </p>
                            <p>3. Verifique via tag HTML (já está no site) ou DNS</p>
                            <p className="text-slate-500 mt-2">
                              O código de verificação já está no site:{" "}
                              <span className="font-mono text-sd-green">Q3iqnEYQT-...</span>
                            </p>
                          </div>
                          <a
                      ...
                      <Card className="bg-[#131921] border-white/10">
                        <CardHeader className="flex flex-row items-center gap-4">
                          ...
                          <div>
                            ...
                            <CardDescription>
                              Eventos, conversões e fluxo de usuários
                            </CardDescription>
                          </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          <div className="grid grid-cols-2 gap-3">
                            <div className="p-3 bg-white/5 border border-white/10 rounded-xl">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-bold text-sd-green">
                                  GTM-TFGQWCQN
                                </span>
                              </div>
                          ...
                          <div className="p-3 bg-sd-black/40 border border-white/5 rounded-lg text-[11px] text-slate-300 space-y-1">
                            ...
                            <p>
                              1. Crie uma propriedade GA4 em{" "}
                              <span className="text-sd-green font-mono">analytics.google.com</span>
                            </p>
                            <p>
                              2. Copie o Measurement ID (ex:{" "}
                              <span className="font-mono text-sd-green">G-XXXXXXXXXX</span>)
                            </p>
                            <p>
                              3. Adicione na Vercel:{" "}
                              <span className="font-mono text-sd-green">
                                VITE_GA4_ID = G-XXXXXXXXXX
                              </span>
                            </p>
                            <p className="text-slate-500 mt-1">
                              Ou configure via GTM → GA4 Configuration Tag
                            </p>
                          </div>
                          <a
                          ...
                          >
                            <Button
                              variant="outline"
                              className="w-full border-white/10 text-white font-bold h-10 hover:bg-white/5"
                            >
                      ...
                      <Card className="bg-[#131921] border-white/10">
                        <CardHeader className="flex flex-row items-center gap-4">
                          ...
                          <div>
                            ...
                            <CardDescription>
                              Captura de leads via formulário do widget
                            </CardDescription>
                          </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                          <div className="p-3 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between">
                            <span className="font-mono text-xs text-slate-300">
                              Widget ID: 6a05e7c2f127bb4126a40721
                            </span>
                            <Badge className="bg-sd-green/20 text-sd-green border-sd-green/30">
                              Ativo
                            </Badge>
                          </div>
                          <div className="p-3 bg-sd-black/40 border border-white/5 rounded-lg text-[11px] text-slate-300 space-y-1">
                            ...
                            <p className="font-mono text-sd-green break-all">
                              {typeof window !== "undefined"
                                ...
                                : "https://sidingdepot.vercel.app"}
                              /api/ghl-webhook
                            </p>
                            <p className="mt-2">
                              Configure no GHL → Settings → Integrations → Webhooks → POST
                            </p>
                          </div>
                        </CardContent>
                      </Card>
                  ...
                  <div className="space-y-8 animate-in fade-in duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <KPICard
                        ...
                        value={metrics?.blogStats?.totalPosts}
                        icon={FileText}
                        color="sd-green"
                        diff="Ativos no site"
                      />
                      <KPICard
                        ...
                        value={metrics?.blogStats?.avgReadingTime}
                        icon={Clock}
                        color="sd-green"
                        diff="Benchmark do setor"
                      />
                      <KPICard
                      ...
                      <CardHeader className="flex flex-row items-center justify-between">
                        <div>
                          <CardTitle className="text-xl font-black text-white uppercase tracking-tight">
                            Ranking de Performance do Blog
                          </CardTitle>
                          <CardDescription className="text-xs">
                            Comparativo detalhado entre os melhores posts
                          </CardDescription>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          className="border-white/10 hover:bg-white/5 h-8 text-[10px] font-black uppercase"
                        >
                                    ...
                                    <div className="flex gap-1 justify-end flex-wrap max-w-[200px] ml-auto">
                                      {art.keywords?.map((kw: string, kidx: number) => (
                                        <Badge
                                          key={kidx}
                                          variant="outline"
                                          className="text-[9px] py-0 px-1 border-white/10 text-slate-400 font-normal"
                                        >
                                          {kw}
                                        </Badge>
                                      ))}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <Card className="bg-[#131921] border-white/10 flex flex-col items-center justify-center p-8 text-center">
                        ...
                        <p className="text-sm text-slate-400 mb-6">
                          Com base na performance das palavras-chave, sugerimos criar conteúdos
                          sobre "Automação de Orçamentos com IA".
                        </p>
                        <Button className="bg-sd-green hover:bg-sd-green-hover text-sd-black font-black uppercase text-xs px-8">
                          Ver Sugestões IA
                        </Button>
                      </Card>
                    </div>
      ...
      <Dialog open={!!selectedPageForLeads} onOpenChange={() => setSelectedPageForLeads(null)}>
        <DialogContent className="bg-[#131921] border-white/10 text-white max-w-2xl">
          ...
          <div className="space-y-6 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
                  {metrics?.topPages
                    ?.find((p: any) => p.path === selectedPageForLeads)
                    ?.leadsBySource?.map((src: any, idx: number) => (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <Badge className="bg-sd-green text-sd-black font-black whitespace-nowrap">
                          {src.count} LEADS
                        </Badge>
                      </div>
                    ))}
                  {(!metrics?.topPages?.find((p: any) => p.path === selectedPageForLeads)
                    ?.leadsBySource ||
                    metrics?.topPages?.find((p: any) => p.path === selectedPageForLeads)
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {(() => {
                    const page = metrics?.topPages?.find(
                      (p: any) => p.path === selectedPageForLeads,
                    );
                    return [
            ...
            <div className="space-y-3">
              <p className="text-xs font-black uppercase text-slate-400">
                Evolução de Leads por Canal
              </p>
              <div className="h-[250px] rounded-xl bg-white/5 p-4 border border-white/5">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={
                      metrics?.topPages?.find((p: any) => p.path === selectedPageForLeads)?.trend ||
                      []
                    }
                  >
                    <CartesianGrid
        ...
        <DialogContent className="bg-[#131921] border-white/10 text-white max-w-4xl max-h-[90vh] overflow-y-auto custom-scrollbar">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              Leads, UTM, palavras-chave e evolução no período selecionado.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                  {(selectedBlogArticle?.leadsBySource || []).map((src: any, idx: number) => (
                    <div
                      ...
                      className="flex items-center justify-between rounded-lg bg-white/5 p-3 border border-white/5"
                    >
                      <span className="text-sm font-bold truncate max-w-[150px]" title={src.source}>
                        {src.source}
                      </span>
                      <Badge className="bg-sd-green text-sd-black font-black">{src.count}</Badge>
                    </div>
                  ))}
                  {(!selectedBlogArticle?.leadsBySource ||
                    selectedBlogArticle.leadsBySource.length === 0) && (
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(selectedBlogArticle?.keywords || []).map((kw: string) => (
                    <Badge key={kw} variant="outline" className="border-white/10 text-slate-300">
                      {kw}
                    </Badge>
                  ))}
                </div>
          ...
          <div className="space-y-2 pb-4">
            <p className="text-xs font-black uppercase text-slate-400">
              Visão Geral de Tráfego vs Leads
            </p>
  ...
  return (
    <Card className="bg-[#131921] border-white/10 relative overflow-hidden group hover:border-sd-green/30 transition-all duration-300">
      ...
      <CardHeader className="pb-2">
        <CardDescription className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-300">
          {title}
        </CardDescription>
  ...
  return (
    <div className="bg-[#131921]/50 border border-white/5 rounded-xl p-4 hover:bg-[#131921] transition-colors">
      ...
      <div className="flex items-end justify-between">
        ...
        {diff ? (
          <span
            className={cn("text-[10px] font-bold", isPositive ? "text-sd-green" : "text-red-400")}
          >
            {diff}
          </span>
        ) : null}
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <div className="mt-6 p-4 bg-sd-green/5 border border-sd-green/20 rounded-xl max-w-md text-left">
                ...
                <ul className="text-[11px] text-slate-300 space-y-1 list-disc list-inside">
                  <li>
                    {lang === "pt"
                      ? "Preencha o widget de chat no site"
                      : "Fill out the chat widget on the site"}
                  </li>
                  <li>
                    {lang === "pt"
                      ? "Configure o webhook no GHL: POST /api/ghl-webhook"
                      : "Configure the webhook in GHL: POST /api/ghl-webhook"}
                  </li>
                </ul>
              </div>
            </div>
              ...
              <TableBody>
                {leads.map((lead) => (
                  <TableRow key={lead.id} className="border-white/5 hover:bg-white/5">
                    ...
                    <TableCell className="font-mono text-[10px] text-slate-400 max-w-[160px] truncate">
                      {lead.page_url || "—"}
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="outline"
                        className="border-sd-green/30 text-sd-green text-[10px]"
                      >
                        {lead.source || "ghl"}
                      </Badge>
                    </TableCell>
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && Array.isArray((window as any).dataLayer)) {
      const meaningful = (window as any).dataLayer
        .filter(
          (e: any) =>
            e.event && e.event !== "gtm.js" && e.event !== "gtm.load" && e.event !== "gtm.dom",
        )
        .slice(-20)
        .reverse();
  ...
  const integrations = [
    {
      ...
      id: "GTM-TFGQWCQN",
      ok: typeof window !== "undefined" && !!(window as any).google_tag_manager,
    },
    {
      name: "Google Analytics 4",
      id: "GA4",
      ok: typeof window !== "undefined" && typeof (window as any).gtag === "function",
    },
        ...
        <Card className="bg-[#131921] border-white/10">
          ...
          <CardContent className="space-y-4">
            {integrations.map((service, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-white/5 rounded-xl">
                ...
                <Badge
                  className={
                    service.ok
                      ? "bg-sd-green/10 text-sd-green border-sd-green/20"
            ...
            <div className="p-4 bg-sd-green/5 border border-sd-green/20 rounded-xl">
              ...
              <p className="text-[11px] text-slate-300 leading-relaxed">{t("debugDesc")}</p>
            </div>
          </CardContent>
        </Card>
      </div>
