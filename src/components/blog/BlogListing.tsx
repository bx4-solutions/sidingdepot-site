            ...
            {featuredPost && (
              <div className="relative group overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 flex flex-col md:flex-row min-h-[450px]">
                <div className="md:w-[55%] relative overflow-hidden bg-gray-100">
                  <img 
                    src={getOptimizedUnsplashUrl(featuredPost.heroImage.url, { width: 1200, height: 675 })} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
              {otherPosts.map((post) => (
                <article key={post.slug} className="group flex flex-col">
                  <Link to="/blog/$slug" params={{ slug: post.slug }} className="relative overflow-hidden rounded-xl aspect-[16/9] mb-6 block bg-gray-100 shadow-md">
                    <img 
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-6 text-[11px] font-bold uppercase tracking-wider text-sd-gray-text/50 border-t pt-4">
                    <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {new Date(post.publishDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                    <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {post.readTime} min</span>
                    <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> Team</span>
                  </div>
                  <Link 
                    to="/blog/$slug" 
