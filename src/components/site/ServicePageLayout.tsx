...
function TrustStrip() {
  ...
  const items = [
    ...
    { label: "4.7★", sublabel: "261 GuildQuality Reviews", href: "https://www.guildquality.com/profile/siding-depot-llc", badge: false },
    { label: `${rating}★`, sublabel: `${totalReviews} Google Reviews`, href: "https://www.google.com/search?q=Siding+Depot+Marietta+GA+reviews", badge: false },
    { label: "4.9★", sublabel: "91 Thumbtack Reviews", href: "https://www.thumbtack.com/ga/marietta/vinyl-siding-install/siding-depot-llc/service/482073672544780291", badge: false },
        ...
        <div className="flex flex-wrap items-center justify-center lg:justify-between gap-x-8 gap-y-0 py-0">
          {items.map((item, i) => {
            const inner = (
              ...
              >
                {item.badge ? (
                  <img src={jamesHardieBadge} alt="James Hardie Elite Preferred" className="h-10 w-auto object-contain" loading="eager" />
                ) : (
                ...
                <div>
                  {!item.badge && <p className="text-sm font-bold text-white leading-none">{item.label}</p>}
...
function StatItem({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center text-center px-6 py-8">
      <span ref={ref} className="font-display text-3xl sm:text-5xl lg:text-6xl" style={{ color: SD_LIME }}>
...
// ─── ProblemSolution ──────────────────────────────────────────────────────────
function ProblemSolution({ headline, points, solution }: { headline: string; points: string[]; solution: string }) {
  return (
    <section className="py-24 lg:py-32 bg-white">
      <div className="mx-auto max-w-7xl px-4 lg:px-8 grid gap-16 lg:grid-cols-2 items-center">
        <div>
          ...
          <h2 className="font-display text-4xl sm:text-5xl text-sd-navy leading-tight mb-6">{headline}</h2>
          <ul className="space-y-3">
            {points.map((p) => (
              <li key={p} className="flex items-start gap-3 text-gray-600">
                <span className="mt-2 h-2 w-2 shrink-0 rounded-full" style={{ background: SD_LIME }} />
...
// ─── ServiceOptions ───────────────────────────────────────────────────────────
function ServiceOptions({ eyebrow, headline, subheadline, options }: {
  eyebrow: string; headline: string; subheadline: string; options: ServiceOption[];
}) {
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {options.map((opt) => {
            ...
            return (
              <button key={opt.id} type="button" onClick={() => setActive(isActive ? null : opt.id)}
                className="group relative overflow-hidden rounded-2xl text-left"
                style={{ aspectRatio: "3/4", outline: isActive ? `2px solid ${SD_LIME}` : "none", outlineOffset: 2 }}>
                ...
                <div className="relative z-10 h-full flex flex-col justify-end p-5">
                  {isActive && (
                    <span className="absolute top-4 right-4 text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"
                      style={{ background: SD_LIME, color: SD_NAVY }}>Selected</span>
                  )}
                  <p className="text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color: SD_LIME }}>{opt.subtitle}</p>
                  <h3 className="text-xl font-bold text-white leading-tight">{opt.title}</h3>
                  <p className="mt-2 text-sm text-white/0 leading-relaxed transition-all duration-300 group-hover:text-white/80 overflow-hidden max-h-0 group-hover:max-h-24">{opt.description}</p>
                  <span className="mt-3 inline-flex items-center gap-1 text-xs font-bold" style={{ color: SD_LIME }}>
        ...
        {activeOpt && (
          <div className="mt-6 rounded-2xl p-6 grid sm:grid-cols-[200px_1fr] gap-6 items-center"
            ...
            <img src={activeOpt.image} alt={activeOpt.title} className="rounded-xl w-full object-cover" style={{ aspectRatio: "4/3" }} />
            <div>
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: SD_LIME }}>{activeOpt.subtitle}</p>
...
// ─── ProcessSection ───────────────────────────────────────────────────────────
function ProcessSection({ eyebrow, headline, subheadline, steps }: {
  eyebrow: string; headline: string; subheadline: string; steps: ProcessStep[];
}) {
  return (
    <section className="py-24 lg:py-32 bg-white">
      <div className="mx-auto max-w-6xl px-4 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-20">
          ...
          <h2 className="font-display text-4xl sm:text-5xl text-sd-navy leading-tight">{headline}</h2>
            ...
            return (
              <div key={step.num} className="relative flex gap-4 pb-6">
                <div className="flex flex-col items-center shrink-0" style={{ width: 36 }}>
                  <div className="h-9 w-9 rounded-full border-4 border-white flex items-center justify-center font-display text-xs font-bold shrink-0"
                    style={{ background: SD_LIME, color: SD_NAVY, boxShadow: "0 0 0 3px rgba(179,209,51,0.22)" }}>
                    {step.num}
                  </div>
                  {!isLast && (
                    <div className="flex-1 w-0.5 mt-2" style={{ background: "rgba(179,209,51,0.3)", minHeight: 24 }} />
                  )}
                </div>
                <div className="flex-1 rounded-2xl p-5"
                  style={{ background: "#f9fafb", border: "1px solid #e5e7eb", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
                  <h3 className="text-base font-bold text-sd-navy leading-snug mb-2">{step.title}</h3>
...
const SHOWCASE_PAIRS: ReadonlyArray<ShowcasePair> = BEFORE_AFTER_PAIRS.map((p, i) => {
  ...
  return {
    ...
    description: project?.description ?? "Full exterior transformation completed by Siding Depot across North Atlanta.",
  };
});
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-14">
          <div>
            ...
            <h2 className="font-display text-4xl sm:text-5xl text-white leading-tight">{projectsLabel}</h2>
          </div>
            ...
            <div className="flex items-center gap-2 mb-8">
              {SHOWCASE_PAIRS.map((_, i) => (
                <button key={i} type="button" onClick={() => setIndex(i)} aria-label={`Project ${i + 1}`}
                  ...
                  style={{ width: i === index ? 24 : 8, height: 8, background: i === index ? SD_LIME : "rgba(255,255,255,0.2)" }} />
              ))}
            </div>
    ...
    <section className="py-24 lg:py-32 bg-white">
      <div className="mx-auto max-w-7xl px-4 lg:px-8 grid gap-16 lg:grid-cols-2 items-center">
        <div className="relative">
          ...
          <img src="/projects/project-3.webp"
          ...
          <div className="absolute -bottom-6 -right-4 rounded-2xl px-6 py-4 shadow-xl text-center"
            ...
            <p className="font-display text-4xl" style={{ color: SD_LIME }}>1,500+</p>
            <p className="text-xs text-white/60 uppercase tracking-widest font-bold mt-0.5">Projects Done</p>
          </div>
        </div>
        <div>
          <span className="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-5"
...
const WHY_US_ITEMS = [
  { Icon: Award, title: "Elite Preferred — Top 2% in the US", desc: "One of fewer than 2% of US installers to hold James Hardie Elite Preferred status. This certification is renewed annually and unlocks warranty coverage no standard contractor can offer." },
  { Icon: ShieldCheck, title: "W-2 Crews — Not a Subcontract Shop", desc: "Every crew member on your job is a Siding Depot W-2 employee. Background-checked, trained, and covered under our insurance. We never hand your project to a third party." },
  { Icon: CheckCircle2, title: "Engineered for Georgia's Climate", desc: "Our materials and methods are selected specifically for Georgia's heat, humidity, and storm season. Every product we specify performs under the conditions your home actually faces." },
  { Icon: FileText, title: "Written Estimates — Price Locked", desc: "Every estimate is detailed, itemized, and binding. The number you see is the number you pay. No surprise change orders, no last-minute additions after the job starts." },
  { Icon: Users, title: "On-Site Project Manager Every Day", desc: "A dedicated Siding Depot project manager is physically on your job site each day. You get a daily progress update — by call or text — no chasing, no guessing." },
  { Icon: ShieldCheck, title: "Licensed, Insured & Financing Ready", desc: "Georgia General Contractor #RBQA006789, fully insured, and an approved GreenSky® financing partner offering 0% APR options. We handle the paperwork so you don't have to." },
] as const;
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid lg:grid-cols-[1fr_1.4fr] gap-16 items-start">
          <div className="lg:sticky lg:top-28">
            ...
            <img src="/projects/project-2.webp" alt="Siding Depot professional crew"
        ...
        <div className="relative rounded-3xl overflow-hidden" style={{ background: SD_NAVY }}>
          ...
          <div className="relative grid lg:grid-cols-2 gap-0 items-center">
            <div className="relative h-72 lg:h-full min-h-[400px] overflow-hidden">
              <img src="/projects/project-5.webp" alt="Siding Depot completed exterior project"
                className="absolute inset-0 h-full w-full object-cover" loading="lazy" />
              <div className="absolute inset-0 lg:hidden" style={{ background: "linear-gradient(to top, #1e2a3a 0%, transparent 60%)" }} aria-hidden />
              <div className="absolute inset-0 hidden lg:block" style={{ background: "linear-gradient(to right, transparent 60%, #1e2a3a 100%)" }} aria-hidden />
            </div>
...
function ServiceHero({ cfg }: { cfg: ServicePageConfig }) {
  return (
    <section className="relative isolate overflow-hidden" style={{ minHeight: "92vh" }}>
      <img src={cfg.heroImage} alt={cfg.heroImageAlt}
        ...
        <div className="max-w-3xl">
          <div className="flex items-center justify-center gap-3 mb-6">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest"
              style={{ background: "rgba(179,209,51,0.15)", border: `1px solid ${SD_LIME}`, color: SD_LIME }}>
          ...
          <h1 className="font-display text-white leading-[0.9] text-4xl sm:text-5xl lg:text-7xl xl:text-8xl">
            ...
            {cfg.heroLine3 && <><br />{cfg.heroLine3}</>}
          </h1>
          <p className="mt-6 text-lg text-white/75 leading-relaxed max-w-2xl mx-auto">{cfg.heroSubtitle}</p>
  ...
  return (
    <div className="flex flex-col">
      ...
      <ProcessSection
        ...
        subheadline={config.processSubheadline ?? "A clear, predictable process from first call to final walkthrough."}
        steps={config.steps}
      />
