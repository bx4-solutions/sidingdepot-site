      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="mt-12 grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {items.map(({ Icon, title, desc }) => (
            <div
              key={title}
              className="group flex flex-col rounded-xl border border-sd-gray-border bg-white p-6 transition-all hover:border-sd-green hover:-translate-y-1 hover:shadow-lg"
            >
              <div className="flex h-11 w-11 items-center justify-center rounded-md bg-sd-green-pale text-sd-navy shrink-0">
                <Icon className="h-5 w-5" strokeWidth={2} />
              </div>
              <div className="mt-4 flex flex-col flex-grow">
                <h3 className="text-lg font-semibold text-sd-black">{title}</h3>
