...
export function HeroQuoteForm({
  ...
  return (
    <div className={bare ? "w-full" : "w-full max-w-[420px] mx-auto rounded-xl bg-white shadow-xl ring-1 ring-black/5 overflow-hidden transition-all hover:shadow-sd-green/5"}>
      {!bare && (
      ...
      <form onSubmit={onSubmit} className={bare ? "grid gap-2.5" : "px-5 py-5 grid gap-3"}>
        <div className="grid gap-1">
          <Label htmlFor="hero-name" className="text-[11px] font-bold text-sd-black uppercase tracking-wider">Full Name *</Label>
          <Input {...register("name")} id="hero-name" placeholder="Your full name" className="h-9 text-sm border-sd-navy/10 focus:ring-2 focus:ring-sd-green/20" />
          {errors.name && <p className="text-[11px] text-destructive font-medium">{errors.name.message as string}</p>}
        </div>

        <div className="grid grid-cols-2 gap-2.5">
          <div className="grid gap-1">
            <Label htmlFor="hero-phone" className="text-[11px] font-bold text-sd-black uppercase tracking-wider">Phone *</Label>
            <Input {...register("phone")} id="hero-phone" type="tel" placeholder="(678) 000-0000" className="h-9 text-sm border-sd-navy/10 focus:ring-2 focus:ring-sd-green/20" />
            {errors.phone && <p className="text-[11px] text-destructive font-medium">{errors.phone.message as string}</p>}
          </div>
          <div className="grid gap-1">
            <Label htmlFor="hero-email" className="text-[11px] font-bold text-sd-black uppercase tracking-wider">Email *</Label>
            <Input {...register("email")} id="hero-email" type="email" placeholder="your@email.com" className="h-9 text-sm border-sd-navy/10 focus:ring-2 focus:ring-sd-green/20" />
            {errors.email && <p className="text-[11px] text-destructive font-medium">{errors.email.message as string}</p>}
          </div>
        </div>

        <div className="grid gap-1">
          <Label htmlFor="hero-city" className="text-[11px] font-bold text-sd-black uppercase tracking-wider">City *</Label>
          <Input {...register("city")} id="hero-city" placeholder="Your city" className="h-9 text-sm border-sd-navy/10 focus:ring-2 focus:ring-sd-green/20" />
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
            {SERVICES_OPTIONS.map((opt) => (
              <div key={opt} className="flex items-center gap-2">
                ...
                />
                <Label htmlFor={`svc-${opt}`} className="text-[13px] text-sd-black cursor-pointer">{opt}</Label>
              </div>
            ))}
          </div>
        ...
        <div className="grid gap-1">
          <Label htmlFor="hero-details" className="text-[11px] font-bold text-sd-black uppercase tracking-wider">Project Details</Label>
          <Textarea {...register("details")} id="hero-details" placeholder="Describe your project..." className="resize-none min-h-[64px] text-sm border-sd-navy/10 focus:ring-2 focus:ring-sd-green/20" rows={2} />
        </div>
        ...
        <div className="flex items-start gap-2 p-2.5 bg-sd-gray-bg/50 rounded-md border border-sd-navy/5">
          ...
          />
          <div className="grid gap-1">
            <Label htmlFor="hero-consent" className="text-[10px] leading-snug text-sd-gray-text cursor-pointer">
