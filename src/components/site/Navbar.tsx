    ...
    <header className="sticky top-0 z-50 w-full backdrop-blur-md bg-sd-gray-bg/80 border-b border-sd-navy/10 shadow-sm transition-all duration-300">
      <div className="mx-auto flex h-nav-mobile sm:h-nav-tablet lg:h-nav-desktop max-w-7xl items-center justify-between gap-3 px-4 lg:px-8">
        ...
        <nav className="hidden lg:flex items-center gap-1 h-full">
          {NAV_LINKS.map((l) => (
            <div key={l.label} className="h-full flex items-center group">
              <div className="flex items-center gap-1 py-2 px-3 relative">
                <Link 
                  to={l.to} 
                  className={cn(
              ...
              {l.sublinks && (
                <div className="opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50" style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', top: '100%' }}>
